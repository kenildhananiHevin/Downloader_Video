...
public abstract class BaseLauncherActivity extends BaseActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (savedInstanceState == null) {
//            ValidationHandler.validateOptions(this, () -> {
                appUpdateManager = AppUpdateManagerFactory.create(BaseLauncherActivity.this);
...
//            });
    ...
    private void launchApiCall() {
        ...
        appConfigApiService.getApp(packageName).enqueue(new Callback<ListModel>() {
            ...
            public void onResponse(@NotNull Call<ListModel> call, @NotNull Response<ListModel> response) {
                Log.d("TAG", "onResponse123: "+new Gson().toJson(response.body()));
                if (response.body() != null && response.isSuccessful()) {
                    try {
                        String res = new Gson().toJson(response.body());
                        config = new Gson().fromJson(res, ListModel.class);
                        ...
                        onApiFailed("Error: Could not parse response!"+e.getMessage());
                    }
                } else {
                    onApiFailed("Error: Failed to fetch response!");
                }
            }
            ...
            public void onFailure(@NotNull Call<ListModel> call, @NotNull Throwable t) {
                t.printStackTrace();
                onApiFailed("Error: Cannot connect to Server!");
            }
        });
    }
    ...
    private void proceed() {
        ...
        if (permissions.length > 0) {
            boolean showAd = !config.initialAppOpen && !config.blockInitialAppOpen;
            checkRunTimePermission(callback, showAd, permissions);
        } else {
            if (config.initialAppOpen || config.blockInitialAppOpen) {
                callback.completed();
            } else {
                showInitialInterstitial(callback);
            }
        }
    }
    ...
    private void setupAdMembers(List<String> limitedArea) {
        ...
        //true = inter first, false = otherwise
        AdsUtility.shouldAlternate = !config.blockInitialAppOpen && config.initialAppOpen;

        try {
            //validating start-screen visibility
            if (config.repeatScreenCount != -1) {
                SharedPreferences pref = PreferenceManager.getDefaultSharedPreferences(this);
                int currentRepeatCount = pref.getInt("repeat_screen_count", config.repeatScreenCount);
                if (currentRepeatCount == 0) {
                    config.screenCount = "";
                } else {
                    pref.edit().putInt("repeat_screen_count", --currentRepeatCount).apply();
                }
            }
    ...
    protected void postInit() {
        ...
        if (config.preloadNative) {
            ...
            AdsUtility.loadAdMobMediumRectangleBannerPreload(this); //initial request
        }

        //if initially app open not required, preload for next use
        boolean appOpenIdValidation = !TextUtils.isEmpty(config.adMob.appOpenId);
        boolean onDemand = config.onDemandAppOpen;
        boolean minimalAppOpen = AdsUtility.minimalAppOpen(); //prevent initial load
        if (appOpenIdValidation && !minimalAppOpen && (!config.initialAppOpen || config.blockInitialAppOpen) && !onDemand) {
    ...
    protected void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        ...
        if (requestCode == REQUEST_CODE_PERMISSION) {
            recheckUpdate();
                ...
                && (resultCode == RESULT_CANCELED || resultCode == ActivityResult.RESULT_IN_APP_UPDATE_FAILED)) {
